...
* Para promessas que não dependem de recursos externos, usar o Promise.all não vai fazer elas serem executadas em paralelo porque, como todas são resolvidas na própria thread do node, tudo acabará endo feito em alguma sequência arbitrária que o motor do node definirá. Paralelimos com promessas do node só podem acontecer, de fato, quando são feitas promessas que são resolvidas por processamentos externos.
